    ...
    public static ByteArrayInputStream leadsToExcel(List<Lead> leads) {
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream();) {
            ...
            for (Lead lead : leads) {
                ...
                row.createCell(3).setCellValue(lead.getLeadType());
                row.createCell(3).setCellValue(lead.getAddress());
                row.createCell(3).setCellValue(lead.getDesignation());
                row.createCell(3).setCellValue(lead.getCompany());
                row.createCell(3).setCellValue(lead.getNote());
            }
